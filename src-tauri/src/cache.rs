...
pub struct GitCache {
    ...
    status_ttl: Duration,
    /// TTL for the dashboard scan status cache — deliberately longer than the
    /// interactive one: the dashboard tolerates a slightly stale count, and
    /// every git write operation invalidates it anyway.
...
impl GitCache {
    ...
    /// Outer `None` = cache miss; inner `None` = a cached failure.
